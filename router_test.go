...
func TestRouter(t *testing.T) {
	...
	for urlsuffix, expectedStatusCode := range getURLAndStatusCodes {
		...
		// extra test for /api/v1/book
		if urlsuffix == "/api/v1/book/" {
			...
			if ok == false {
				t.Errorf("804607066 expected book payloads for /api/v1/book %+v", bookList)
			} else {
				if len(bookList) != 2 {
					t.Errorf("804607067 expected 2 book payloads for /api/v1/book")
